...
def listen_print_loop(responses):
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            # by using Google Translation API
            translation = translate_client.translate(
                transcript,target_language=target)
            sttText = translation['translatedText']
            print('{}'.format(translation['translatedText']))

            # Save translated text into a mp3 file by using
            # Google text to speech API, gtts.
            # And play it with playsound package
            tts = gTTS(text = sttText, lang='ja')
...
def main():
    ...
    playsound.playsound('Translation_ja_start.mp3')
